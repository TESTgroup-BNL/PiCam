...
image_path <- file.path(here::here(),"Examples/Arctic_vegetation_phenology/example_data/alaska_picam_14/images")
...
fall_vi_extract$alder_roi1_fall$gcc <- (fall_vi_extract$alder_roi1_fall$g.av) / 
  (fall_vi_extract$alder_roi1_fall$r.av + fall_vi_extract$alder_roi1_fall$g.av + 
     fall_vi_extract$alder_roi1_fall$b.av)

# show the fall timeseries plot and write to the ROI folder
par(mfrow=c(1,1), mar=c(4,4.6,0.3,0.4), oma=c(0, 0, 0, 0))
plot(lubridate::yday(fall_vi_extract$alder_roi1_fall$date),
     fall_vi_extract$alder_roi1_fall$gcc,
...
fall_vi_extract_df_output <- fall_vi_extract[[roi_name]][lubridate::year(fall_vi_extract$alder_roi1_fall$date)==2021,]
head(fall_vi_extract_df_output)
...
with(temp$alder_roi1, plot(date, gi.av, pch=20, col='green',
...
temp$alder_roi1$gcc <- (temp$alder_roi1$g.av)/(temp$alder_roi1$r.av+
                                                 temp$alder_roi1$g.av+
                                                 temp$alder_roi1$b.av)
plot(temp$alder_roi1$date,temp$alder_roi1$gcc,
     ylim=c(0.2,0.45))
